...
Laravel 13 на PHP 8.4 отдаёт JSON API, фронтенд — отдельный SPA на Vue 3 и TypeScript (Vite 8, Tailwind 4). База — SQLite. Тесты — Pest, форматирование — Pint, типы — vue-tsc.

Для фронтенда нужен Node 22 (`nvm use 22`). На Node 16 Vite не запустится.

API описан контрактом в `contract/main.tsp` (TypeSpec). Из него генерируются: OpenAPI, клиентский SDK для Vue, PHP-типы и `routes/api.php`. Сгенерированное коммитится и руками не правится (ADR-0011).
...
Вне модулей: `contract/` — исходник контракта и сгенерированная OpenAPI; `app/Contract/Generated/` — PHP-типы из контракта; `resources/js/` — SPA, внутри `resources/js/api/` — сгенерированный клиент.
...
| Перегенерировать всё из контракта | `npm run contract` |
